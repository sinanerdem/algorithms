...
-----------------------------------------------------------------*/
template <typename T>
dstack<T>::dstack(bool d, int size) :_display(d), _sp(0), _stack(d, size), _minp(0), _minarr(false, 2){
  if (display()) {
...
void dstack<T>::for_each_element_of_stack_from_top_to_bottom(void(*pf) (T& c))  {
  for (int i = _sp - 1; i >= 0; i--) {
    pf(_stack[i]);
    if (_stack[i] < _stack[_minarr[_minp]]){
    	_minarr[++_minp]=i;
    }
  }
}
/*----------------------------------------------------------------
min: returns the minimum data stored in the stack
-----------------------------------------------------------------*/
template <typename T>
T dstack<T>::min(){
  if (isempty()) {
